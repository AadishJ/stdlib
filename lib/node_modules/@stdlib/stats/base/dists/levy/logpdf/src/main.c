...
*
* @example
* double y = stdlib_base_dists_levy_logpdf( 9.0, 0.0, 1.0 );
* // returns ~-5.931
*/
double stdlib_base_dists_levy_logpdf( const double x, const double mu, const double c ) {
	double z;
	if ( stdlib_base_is_nan( x ) || stdlib_base_is_nan( mu ) || stdlib_base_is_nan( c ) || c <= 0.0 || ( !stdlib_base_is_finite( x ) && x < 0.0 ) ) {
		return 0.0 / 0.0; // NaN
	}
	if ( x <= mu ) {
