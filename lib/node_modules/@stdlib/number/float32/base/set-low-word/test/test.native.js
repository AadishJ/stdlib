...
tape( 'the function returns a number', opts, function test( t ) {
	var y = setLowWord( pow( 2, 23 ), 5 );
	t.equal( typeof y, 'number', 'returns a number' );
	t.end();
});

tape( 'the function sets the lower order word of a single-precision floating-point number', opts, function test( t ) {
	var actual;
	var values;
	var high;
	var low;
	...
	for ( i = 0; i < values.length; i++ ) {
		...
		for ( j = 0; j < 10; j++ ) {
			...
			t.equal( actual, low, 'returned single has the same lower order word as ' + v );

			// Higher order bits:
			actual = toBinaryString( y ).substring( 0, 32 );
			t.equal( actual, high, 'returned single has the same higher order word as ' + v );
		}
	}
	t.end();
});
